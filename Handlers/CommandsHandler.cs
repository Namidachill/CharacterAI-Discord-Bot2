...
namespace CharacterAI_Discord_Bot.Handlers
{
    public class CommandsHandler : HandlerService
    {
        ...
        private async Task HandleMessageAsync(SocketMessage rawMsg)
        {
            ...
            bool containsFox = message.Content.Contains("‚«‚Â‚Ë");
            ...
            bool gottaExecute = containsFox || hasMention || hasPrefix;
        ...
        private string AddTimestamp(string text, SocketMessage message, bool includeLabel = true)
        {
            var timestamp = message.CreatedAt.LocalDateTime;
            string timestampLabel = includeLabel ? "Timestamp: " : "";
        ...
        private async Task TryToCallCharacterAsync(SocketCommandContext context, Models.DiscordChannel currentChannel, bool inDMorPrivate)
        {
            ...
            if (amode == 1 || amode == 3 || amode == 4 || inDMorPrivate)
                text = AddUsername(text, context);
            if (amode == 2 || amode == 3)
                text = AddQuote(text, context.Message);
            if (amode == 4)
            {
                bool includeLabel = false; // ƒ‰ƒxƒ‹‚ðŠÜ‚ß‚È‚¢‚æ‚¤‚ÉÝ’è
                string timestampText = AddTimestamp("", context.Message, includeLabel);
                text = $"{text}\n{timestampText}";
            }
